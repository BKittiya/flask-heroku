...
def create_model_pred(prd, prv, mth):
    # Predict AOP
    data = pd.read_csv(
        "C:/Users/BEER/OneDrive/เดสก์ท็อป/RS_FINAL/amt_of_prd_per_area/all_for_" + cvt_eng[prd].lower()+".csv")
    ...
    exogenous_features = list(corr[(abs(corr) >= 0.8) & (
        corr.index != 'Amount_of_product_per_area')].index)
    df = data[exogenous_features + ['temp_AM', 'rain_AM', 'humid_AM',
                                    'temp_PM', 'rain_PM', 'Amount_of_product_per_area']]

    # normalize the dataset
    scaler = MinMaxScaler(feature_range=(0, 1))
    df['Amount_of_product_per_area'] = scaler.fit_transform(
        np.array(df['Amount_of_product_per_area']).reshape(-1, 1))
    print(df.shape)
    reg = LinearRegression().fit(df.drop(
        columns=['Amount_of_product_per_area']), df['Amount_of_product_per_area'])

    # Predict
    fore_var = pd.read_csv(
        "C:/Users/BEER/OneDrive/เดสก์ท็อป/RS_FINAL/For_Forecast/all_forecast_aop.csv")
    pred_var = fore_var[(fore_var['province'] == prv) &
                        (fore_var['year'] == current_year)]
    pred_var = pred_var[exogenous_features + ['temp_AM',
                                              'rain_AM', 'humid_AM', 'temp_PM', 'rain_PM']]
    predict_aop = scaler.inverse_transform(
        reg.predict(pred_var).reshape(-1, 1))

    # Predict Price
    file_prd = "C:/Users/BEER/OneDrive/เดสก์ท็อป/RS_FINAL/price/ALL_FOR_" + \
        cvt_eng[prd] + "_PRICE.csv"
    ...
    exogenous_features = list(
        corr[(abs(corr) >= 0.2) & (corr.index != 'price')].index)
    ...
    # Predict
    fore_var = pd.read_csv(
        "C:/Users/BEER/OneDrive/เดสก์ท็อป/RS_FINAL/For_Forecast/all_forecast_price_new.csv")
    pred_var = fore_var[(fore_var['year'] == current_year)
                        & (fore_var['month'] == cvt_mth[mth])]
    pred_var = pred_var[exogenous_features]
    predict_price = scaler.inverse_transform(
        reg2.predict(pred_var).reshape(-1, 1))
